...
**When you fix a bug in a shared tab, apply it to all four folders** — there is no shared source; `diff` the folders to check drift. The only intentional drift is `Wolpac-ID04/loop.ino`: analog thresholds `<1000` / `>3000` instead of `<300` / `>450`. Everything else must be byte-identical across the four apart from the variant lines above; the reference copy is `Garen_ID03` (the lab test board), and the port to the other three was done by generating each main sketch from Garen's with only those lines substituted (2026-09-21; pre-port originals in `backup/`).
